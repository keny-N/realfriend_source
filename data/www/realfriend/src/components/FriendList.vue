    ...
    <div class="friendinsert">
      <ul>
        ...
        <li v-for="friend in friends"  class="rounded border border-dark">
          ...
          <div>
          {{ friend[2]}}
          <router-link :to="{name:'GameBody',params:{friendid:friend[0]}}">
          ...
          <div>
            ...
            <button v-on:click="fridele(friend[0])">さくじょおおおお</button>
          </div>
        </li>
      </ul>
...
<script>
    export default {
      ...
      data: function () {
        return {
          //getUrlでは適当なユーザーのフレンドを一覧として取得するためのURL
          getUrl:'https://abwp9ub4n8.execute-api.ap-northeast-1.amazonaws.com/realfriend/friends/all/1',
          //deleUrlでは削除したいフレンドのIDを指定することでデータベースから削除されるURL
          deleUrl:'https://abwp9ub4n8.execute-api.ap-northeast-1.amazonaws.com/realfriend/friends/one/',
          friends:[],
        };
      },
      mounted() {
      ...
      methods: {
        //フレンド削除のやつきっと消える
        fridele(value){
          this.axios.delete(this.deleUrl+Number(value)
